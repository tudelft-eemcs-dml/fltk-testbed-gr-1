...
class HistogramFeatureSet(FeatureSet):
    ...
    def extract(self, data):
        ...
        if self.datatype is DataFrame:
            assert all(
                [c in list(data) for c in self.cat_attr_names]
            ), "Missing some categorical attributes in input data"
            assert all(
                [c in list(data) for c in self.num_attr_names]
            ), "Missing some numerical attributes in input data"
            ...
            for aidx, attr in zip(self.cat_attr_idx, self.cat_attr_names):
                ...
                features.extend(F.tolist())
        assert (
            len(features) == self.nfeatures
        ), f"Expected number of features is {self.nfeatures} but found {len(features)}"
